...
<html>
  ...
  <body>
    ...
    <sql:query var="rs" dataSource="${serverDataSource}" sql="select count(*) count from mysql.help_topic" scope="page"  />
    <c:forEach var="row" items="${rs.rows}" >
       <jsp:setProperty name="pagination" property="recordCount" value="${row.count}" />
    </c:forEach>


    <sql:query var="rs" dataSource="${serverDataSource}" 
               startRow="${pagination.firstResult}" maxRows="${pagination.pageSize}"
               sql="select * from mysql.help_topic"  scope="page"  />    
    <table>
      <tr class="title" >
        <td>Help_ID</td>
        <td>Name</td>
